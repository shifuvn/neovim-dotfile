...
autocmd('BufWritePre', { group = new_augroup('trailing_whitespace'), command = '%s/\\s\\+$//e' })
...
autocmd('BufWritePre', {
    group = new_augroup('auto_make_dir'),
    callback = function(event)
...
autocmd('BufReadPost', {
    group = new_augroup('goto_last_loc'),
    callback = function(event)
...
autocmd({'BufLeave', 'FocusLost', 'InsertEnter', 'CmdLineEnter', 'WinLeave'}, {
    group = new_augroup('auto_redraw_buffer'),
    pattern = '*',
    callback = function ()
        if vim.o.nu then
            vim.cmd.redraw()
